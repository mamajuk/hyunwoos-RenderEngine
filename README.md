...
때문에 개발의 중간시점부터 오직 ***3D모델링 랜더링*** 및 ***스키닝 애니메이션의 구현*** 에만 충실하도록 설계 범위를 최소화했습니다. 일단 목표는 달성했기에 당분간은 확장할 예정은 없지만, 추후 ```.pmx```, ```.vmd```의 모프나 카메라 모션을 구현하거나, ```.fbx```, ```.obj``` 와 같은 파일의 임포트를 지원하거나, ```Hiererchy Window``` 및 ```Multi ViewPort```등을 천천히 구현해볼 예정입니다.
