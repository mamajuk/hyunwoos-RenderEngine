...
●```pmx``` 모델링은 ```IK bone```과 ```display bone```( 실제 랜더링되는 스킨이 영향받는 본 )이 따로 존재하며, pmx의 IK 데이터의 각 링크본들은 IK 본의 인덱스를 사용합니다. 따라서 ```hyunwoo's RenderEngine```은 각 링크본에서 가장 가까운 다른 본의 인덱스를 찾아 교체하여, IK 적용시 디스플레이 본이 움직이도록 했습니다. 이는 ```pmx``` 모델링의 바인딩 포즈에서 display bone과 ik bone가 서로 비슷한 위치에 있다는 것을 가정하여 적용한 것이기 때문에, 일부 ```pmx``` 모델링에 IK를 적용할 경우 본이 뒤틀리는 경우가 있을 수 있습니다.
