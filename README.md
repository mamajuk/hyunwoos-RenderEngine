...
●```pmx``` 모델링은 계층구조를 ```IK bone```과 ```Display bone``` 두 개로 유지합니다. 여기서 ```pmx``` 모델링 내부의 IK Link들은 모두 ```IK bone```을 사용하기 때문에, IK를 적용해도 실제 버텍스가 움직이지 않습니다. 따라서 ```PmxImport::Import()``` 는 각 링크본들에서 가장 가까운 본을 찾아 교체하는 식으로 IK 정보를 읽어들입니다. 이는 대부분의 ```pmx``` 모델링의 바인딩 포즈에서 ```Display bone```과 ```IK bone```이 비슷한 위치에 있다는 것을 가정하여 적용한 것이기 때문에, 일부 ```pmx``` 모델링에 IK를 적용할 경우 뒤틀리는 상황이 발생할 수 있습니다.

●```PmxImporter::Import()```는 임포트할 ```pmx``` 모델링에서 사용되는 모든 텍스쳐 파일들을 ```.png``` 확장명으로 임포트합니다. 따라서 사용자는 임포트할 ```pmx``` 모델링에서 사용되는 모든 텍스쳐 파일들을 ```.png``` 확장명으로 미리 변환시켜놓아야 정상적으로 임포트할 수 있습니다.
